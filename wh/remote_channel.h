 ...
 * The server starts subtasks, pubslishes a channel interface, and the 
 ...
 *   where the arguments of taskClientInit are those received by main (int argc, const char *argv[]).
 *
 *   On the server side, the program name argv[0] will be  used below to construct robust remote client calls
 *   On the client side, taskClientInit will consume the arguments added implicitely by taskDispatch (see below)
 *   and used by the remote_channel library to communicate with the server, and leave untouched
 *   the parameters specific to the application (second argument of taskDispatch described below) 
 ...
 *
 * The first parameter is the name of a program. If the name starts with /, as in /bin/grep, the name is not modified,
 * otherwise the executable is searched first in the same directory where the server executable resides, then
 * elsewhere on the path. Finally, is the executable is not found, the server stops and reports a FATAL ERROR.
 * These conventions may appear unusual, but the allows robust portable programming. Indeed, it is very likely 
 * that the server and the client program were developped at the same time and reside in the same directory, 
 * or have a known relative path, but once the program is put in production, the absolute location of the executables 
 * cannot be known in advance. A convenient way to maintain agreement between the server and client programs, is
 * to create a single executable, and to recursivelly call the server executable, taskDispatch (0, ...) with
 * modified arguments. This guarantees that server and client always remain synchrone during the
 * development phase and, in particular, agree on the naming of the remote communication channels. 
 ...
 *         // In the program which called taskDispatch()
	   ...
	   TASK *task = taskClientCreate () ;
	    ...
	    // such that the same type of structure ir written and read in a channel of a given name
  ...
  * on distinct machines. Thus, it would be meaningless for a remote channel to transmit pointer types.
  * Latency may be highly variable, and it is probably a good idea
  * to transmit rather large structure. On the other hand, on a large hardware architecture
  ...
  *  and is  o longer listening on the client writer channel.
  ...
  *  tS = taskServerCreate (...) ;
  *  t1 = taskDispatch (...) ;
  *  (not yet available)  taskSwitchContext (tS)  ;   // close communications with and kills t1
...
void taskSwitchContext (TASK *task) ; /* does not yet work */
