...
/* $Id: mystdlib.h,v 1.9 2011/10/26 19:03:18 mieg Exp $ */
...
#if defined(POSIX) || defined(LINUX) || defined(SOLARIS) || defined(OPTERON) || defined(SGI) || \
  defined(HP) || defined(WIN32) || defined(INTEL_SOLARIS) || defined (CYGWIN)
...
#if !(defined(MACINTOSH)  || defined(SOLARIS) || defined(POSIX)   || defined(OPTERON) || defined(WIN32) || defined (INTEL_SOLARIS))
...
#else
#if ! defined(POSIX) && ! defined(SOLARIS) && !defined (INTEL_SOLARIS) && !defined (OPTERON) && ! defined (MAC_X)
...
#define HAVE_FSYNC
	/*
	* define this if your system has the fsync() system call.
	* Right now, all systems I test on have it, so I have not
	* made it conditional.
	*/
