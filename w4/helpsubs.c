...
/************************************************************/
static char     *makeHtmlIndex (AC_HANDLE handle);
static char     *makeHtmlImagePage (const char *link, AC_HANDLE handle);
static HtmlNode *parseHtmlText (char *text, AC_HANDLE handle);
...
QueryRoutine helpOnRegister (QueryRoutine func)
...
char *helpSetDir (char *dirname)
{
  if (dirname)
    ...
    {      
      char *cp = filGetFullPath ("whelp", 0) ;
      strcpy (helpDir, cp) ;
      messfree (cp) ;
  ...
  while (TRUE)
    {
      ...
      {
	char *cp = filGetFullPath(helpGetDir(), 0) ;
...
HtmlPage *htmlPageCreate (const char *helpFilename)
/* complemeted by htmlPageDestroy */
{
  ...
  if (strcmp(helpFilename, "?") == 0)
    {
      page = (HtmlPage *) messalloc (sizeof(HtmlPage));
...
HtmlPage *htmlPageCreateFromFile (FILE *fil)
{
  ...
  page->htmlText = (char *) halloc ((fileSize + 1) * sizeof(char), page->handle);
...
static char *makeHtmlImagePage (const char *link, AC_HANDLE handle)
{
  ...
  len = 0;
  cp = filGetFileName(link, 0) ;
  len = 7+6+strlen(cp)+8+10+strlen(link)+2;
...
static HtmlNode *makeNode (HtmlNodeType type, AC_HANDLE handle)
...
static BOOL parseHtml (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
...
static BOOL parseHead (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
...
static BOOL parseBody (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
...
static BOOL parseComment (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node ;
  int len ;
  char *start ;
...
static BOOL parseTitle (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node ;
  int len, numspaces=0 ;
  char *start ;
...
static BOOL parseHeader (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
  int level ;
...
static BOOL parseCode (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
...
static BOOL parseBold (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
...
static BOOL parseStrong (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
...
static BOOL parseItalic (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
...
static BOOL parseText (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node ;
  int len ;
  char *start ;
...
static BOOL parseHref (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
...
static BOOL parseImage (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node ;
			   ...
			   AC_HANDLE   handle)
{
  ...
  *cp += 4 ;
  ...
  node->left = leftnode ;
  node->lstyle = (HtmlListType )lstyle ;
...
static BOOL parseList (int style, char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode ;
  ...
  *cp += 4 ;			/* skip <UL> */
  ...
  parseListItem ((HtmlListType)style, cp, &leftnode, handle);
  ...
  node->left = leftnode ;
  node->lstyle = (HtmlListType) style ;
...
static BOOL parseSection (char **cp, HtmlNode **resultnode, AC_HANDLE handle)
{
  HtmlNode *node, *leftnode, *rightnode ;
