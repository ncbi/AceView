...
/* $Id: texthelp.c,v 1.4 2017/03/02 22:09:01 mieg Exp $ */
...
BOOL helpPrint (char *helpFilename)
 ...
 if ((page = htmlPageCreate (helpFilename)))
  ...
  for (i = 0, x = 0 ; i < arrayMax(dirList) ; i++)
	...
	continue ;
      if (helpFilename)
	{
	  char *cq = filGetFileName(helpFilename, 0) ;
	  if (strncasecmp(cq, cp, strlen (cq)))
	    continue;
	  messfree (cq) ;
	}
...
/* part specific to the text-help system, which uses freeOut
   to print parsed HTML as plain text */
{
  int i, len ;
  char *cp, *start ;
  static BOOL 
