...
/* $Id: fmapcontrol.c.rd,v 1.1.1.1 2002/07/19 20:23:02 sienkiew Exp $ */
...
static LOOK fMapCreateLook (KEY seq, int start, int stop);
...
#define STL_STATUS_0 
  ...
  mapInsertCol (map, -90.0, TRUE, "Sequences & ends", fMapShowCanonical) ;
  ...
  mapInsertCol (map, 3.5, FALSE, "Gene Names", fMapShowGeneNames) ;
...
/* frame dependent stuff ends */
  ...
  mapInsertCol (map, 6.5, TRUE, "Brief Identifications", fMapShowBriefID) ;
#else
  mapInsertCol (map, 6.5, FALSE, "Brief Identifications", fMapShowBriefID) ;
#endif
  mapInsertCol (map, 7.0, TRUE, "Text Features", fMapShowText) ;
}
...
void fMapDraw (LOOK look, KEY from)
{
 ...
 restart:
  ...
  look->min = look->map->centre - (halfGraphHeight-2)/absMag ;
  look->max = look->map->centre + (halfGraphHeight-2)/absMag ;
...
static LOOK fMapCreateLook (KEY seq, int start, int stop)
{
  ...
  while (Seq)
	...
	BSunit *uu ;
	/* find parent */
	 if (!bsFindTag (Seq, str2tag("S_Parent")) ||
  ...
  look->origin = look->zoneMin - startOrig + 1 ;
     ...
     /* this is being used a DisplayFunc called by display() */
{
  ...
  */
  while (bsGetKey (Seq, _Source, &parent))
    { bsDestroy (Seq) ;
      if (!(Seq = bsCreate(parent)) ||
	  !bsFindKey (Seq, _Subsequence, seq) ||
	  !bsGetData (Seq, _bsRight, _Int, &x1) ||
  ...
  look = fMapCreateLook (seq, start, stop) ;
  mapGifMap = 0 ;
  if (!look)
...
{ 
  int i, redo = 0, nx, ny, orig = -999999 ;
  SEG *seg, *seg1 ;
...
static void addOldSegs (Array segs, Array oldSegs)
{			/* selective for CALCULATED segs */
  ...
  for (i = 1 ; i < arrayMax(oldSegs) ; ++i)
	...
	case SPLICE3: case SPLICE3_UP:
	  ...
	  if (method(seg->key)->flags & METHOD_CALCULATED)
	    { seg->x1 -= offset ;  
	      seg->x2 -= offset ;
...
BOOL fMapConvert (LOOK look, BOOL force)
{
  ...
  BITSET_MAKE_BITFIELD		/* Add bitField array */
  static Array s_children = 0 ;
  ...
  M_Pseudogene = defaultSubsequenceKey ("Pseudogene", DARKGRAY, 2.0, TRUE) ;  /* mieg dec 27. was wrong arg order */
...
#define POS_TO_SEG1	if (isDown) \
  ...
  mainActivity ("Recalculating DNA window") ;
...
/* recurses through all sequences since they get added in turn */
    { seg = arrp(segs,iseg,SEG) ;
      if (seg->type == SEQUENCE || seg->type == TRANSCRIPT)
	isDown = TRUE ;
      else if (seg->type == SEQUENCE_UP || seg->type == TRANSCRIPT_UP)
	isDown = FALSE ;
      else
	...
	{ for (i = 0 ; i < arrayMax(s_children) ; i += 4)
      ...
      else if (bsFindTag (obj, _Subsequence) && 
	...
	if (bsGetData (obj, _bsRight, _Float, &sinf->score))
	  sinf->flags |= SEQ_SCORE ;
         ...
         /* source exons */
      pos1 = 0 ;
      if (bsFindTag (obj, _Source_Exons) && bsFlatten (obj, 2, units))
	{ dnaExonsSort (units) ;
	...
	{ for (i = 0 ; i < arrayMax(units) ; i += 3)
		...
		continue ;
	      seg1 = arrayp (segs,nsegs++,SEG) ; seg = arrp(segs,iseg,SEG) ;
	      seg1->key = seg1->parent = seg1->source = u[0].k ;
	      seg1->data.i = 0 ;
	      if (!u[1].i || !u[2].i)
		...
		{ pos1 = u[1].i ; pos2 = u[2].i ;
		  ...
		  if (seg1->x2 < 0 || seg1->x1 >= look->length)
		    ...
		    { 
		      KEYSET tmp = queryKey (seg1->key, ">Annotations Fully_edited") ; 
		      if (keySetMax (tmp))
			seg1->data.i = 0x1 ;
		      keySetDestroy (tmp) ;
	...
	{ 
	  ...
	  for (i = 0 ; i < arrayMax(units) ; i += 4)
		...
		continue ;
	      ...
	      seg1->parent = seg->key ; seg1->source = parent ;
	      seg1->type = (isDown) ? TRANSCRIPT : TRANSCRIPT_UP ;
      ...
      pos1 = 0 ;
      if ((seg->type | 0x1) == TRANSCRIPT_UP &&
	  bsFindTag (obj, _Assembled_from) && bsFlatten (obj, 5, units))
	{ 
	  seg1 = arrayp (segs,nsegs++,SEG) ; seg = arrp(segs,iseg,SEG) ;
	  ...
	  for (i = 0 ; i < arrayMax(units) ; i += 5)
		...
		continue ;
	      ...
	      POS_TO_SEG1 ;
	      seg1->key = arr(units, i+2, BSunit).k ;
	      seg1->parent = seg1->key ; seg1->source = parent ;

	      pos1 = arr(units, i + 3, BSunit).i ;
	      pos2 = arr(units, i + 4, BSunit).i ;
	      /* reserve bit 31 and 30 for polyAinfo */
	      /* reserve bit 29 for ambiguous, 28 free and 30 for polyAinfo */
...
#define POS_PROCESS(Z,Z_UP) \
    ...
    posConvert (seg1, pos1, pos2) ;
         ...
         /* Oligos, mieg */ 
      if (bsFindTag (obj, _Oligo) && bsFlatten (obj, 3, units))
	for (i = 0 ; i < arrayMax (units) ; i += 3)
	  { OBJ obj1 ;
	    seg1 = arrayp (segs,nsegs++,SEG) ; seg = arrp(segs,iseg,SEG) ;
	    seg1->source = parent ;
		...
		seg1->data.i |=  0x4000 ; /* old */
         ...
         /* Alleles */
      if (bsFindTag (obj, _Allele) && bsFlatten (obj, 4, units))
	for (i = 0 ; i < arrayMax (units) ; i += 4)
	  { char *string ;
	    seg1 = arrayp (segs,nsegs++,SEG) ; seg = arrp(segs,iseg,SEG) ;
	    seg1->source = parent ;
	...
	if (bsFindTag (obj, _Homol) && bsFlatten (obj, 9, units))
	...
	if (bsGetKey (obj, str2tag("Match_table"), &key) && 
	    (ma = arrayGet (key, MATCH, matchFormat)))
	  { FLAGSET fBury = flag ("Match", "Bury") ;
		...
		continue ;
	      seg1 = arrayp (segs,nsegs++,SEG) ; seg = arrp(segs,iseg,SEG) ;
	      seg1->source = key ; /* the match table key */
	      seg1->key = seg1->parent = m->key ;
	      seg1->data.i = arrayMax(look->homolInfo) ;
	      hinf = arrayp(look->homolInfo, seg1->data.i, HOMOLINFO) ;
	      hinf->method = m->meth ;
	      hinf->score = m->score ; 
	      bitSet (look->homolFromTable, seg1->data.i) ;
	      if (m->t1 > m->t2)
		{ pos1 = m->q2 ; pos2 = m->q1 ;
			 ...
			 DICT *sourceSet, DICT *featSet)
{
  ...
  if (!isList)
    { freeOutf ("##gff-version %d\n", version) ;
      freeOutf ("##date %s\n", timeShow (timeParse ("today"))) ;
      freeOutf ("##sequence-region %s %d %d\n", name(seqKey), 
...
void* fMapGifGet (void *opaqueLook)
{
  ...
  if (!lexword2key (word, &key, _VSequence))
    { freeOutf ("// gif sequence error : Sequence %s not known\n", word) ;
      goto usage ;
    }
  ...
  if (x1 < x2 || (x1 == 1 && x2 == 0))
    look = fMapCreateLook (key, x1, x2) ;
  else
    look = fMapCreateLook (key, x2, x1) ;
  mapGifMap = 0 ;
				/* set the zone */
  if (look)
    {
 ...
 usage:
  freeOutf ("// gif seqget error: usage: seqget sequence [-coords x1 x2]\n") ;
  return 0 ;
}
...
usage:
  freeOutf ("// gif seqactions error: usage: SEQACTIONS [-dna] [-gf_features] [-hide_header] [-rev_comp]\n") ;
  fMapGifLook = 0 ; mapGifMap = 0 ; 
}
...
void fMapGifColumns (void* opaqueLook)
{
  ...
  while (freecheck ("w"))
	...
	{ word = freeword() ;
	  if (!word) goto usage ;
	  mapColSetByName (word, TRUE) ;
	}
      else if (!strcmp (word, "-off"))
	{ word = freeword() ;
	  if (!word) goto usage ;
	  mapColSetByName (word, FALSE) ;
	}
      else
	goto usage ;
    }
