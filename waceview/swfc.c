...
static void s_endSWF()
{
    ...
    { /* mieg 2007_01_18 : pass an open file descriptor rather than a file name */
      int nPipe = 0 ; char cc ;
      if(do_cgi || !strcmp(filename, "-"))
	fi = fileno(stdout);
      else if (sscanf (filename, "pipeout_%d%c", &nPipe, &cc) == 1)
	fi = nPipe ;
      else
...
{
  static SWFFONT*font = 0; /* mieg, made it static so we do not have to repeat it */
    ...
    if(!font)
      syntaxerror("font must be specified for at least once in .edittext") ; /* mieg */
...
static double parseNameOrTwip(const char*s)
{
    ...
    if(defines_initialized) {
      l = (char *)dict_lookup(&defines, s) - (char *) 0 ; /* mieg 2010_10_18, bad casting */
    }
    if(l) {
        return (double)(*(int*)&define_values.buffer[l-1]) ; /* mieg 2010_10_18 .buffer is a char*
								so define_values.buffer[l-1] is a char
								so &define_values.buffer[l-1] is a char*
								then we want to return a double
								now this is weird since we actually return an address

								why not return a char* in this familly of functions

						     i cannot read that mess but it cannot possibly be correct
						     since we are supposed ot return a double
						     I add an external cast to a double
						  */
    } else {
...
int parseColor2(const char*str, RGBA*color)
{
    ...
    /* mieg 2007-03-02 expanded the color table to include acedb color names */
    struct {unsigned char r,g,b;char*name;} colors[] =
    {
      ...
      , {0,255,255,"CYAN"}
      , {255,0,255,"MAGENTA"}
      , {255,113,113,"LIGHTRED"}
      , {0,201,196,"LIGHTGREEN"}
      , {133,159,255,"LIGHTBLUE"}
      , {204,0,51,"DARKRED"}
      , {0,153,153,"DARKGREEN"}
      , {22,0,200,"DARKBLUE"}
      , {255,209,209,"PALERED"}
      , {201,255,209,"PALEGREEN"}
      , {205,236,255,"PALEBLUE"}
      , {255,255,195,"PALEYELLOW"}
      , {189,255,255,"PALECYAN"}
      , {255,200,255,"PALEMAGENTA"}
      , {168,40,0,"BROWN"}
      , {255,61,0,"ORANGE"}
      , {255,214,201,"PALEORANGE"}
      , {164,0,164,"PURPLE"}
      , {161,94,228,"VIOLET"}
      , {228,209,247,"PALEVIOLET"}
      , {150,150,150,"GRAY"}
      , {235,235,235,"PALEGRAY"}
      , {255,0,92,"CERISE"}
      , {86,178,222,"MIDBLUE"}
      , {255,125,255,"LIGHTMAGENTA"}
      , {127,255,255,"LIGHTCYAN"}
      , {123, 35,211,"DARKVIOLET"}
      , {65,106,255,"LAVANDER"}
      , {255,250,250,"snow"}
      ...
      , {0,255,255,"cyan"}
      , {127,255,212,"aquamarine"}, {0,255,0,"green"}
      ...
      , {199,159,239,"LIGHTVIOLET"}
      , {0,215,210,"DARKCYAN"}
      , {255, 170, 143,"LIGHTORANGE"}
};
    a=255;r=g=b=0;
    ...
    for(t=0;t<sizeof(colors)/sizeof(colors[0]);t++)
      if(!strncasecmp(str, colors[t].name, len)) { /* mieg 2007_03_02 make color names NON case sensitive */
	    r = colors[t].r;
	    g = colors[t].g;
	    b = colors[t].b;
	    a = alpha;
...
SPOINT getPoint(SRECT r, const char*name)
{
    ...
    if(points_initialized)
      l =  (char*) dict_lookup(&points, name) - (char*) 0; /* mieg, you must substract 2 pointers to get an int */
    if(l==0) {
...
static int c_define(map_t*args)
{
    ...
    dict_put(&defines, name, (char *)NULL + pos + 1); /* mieg 2010_10_18 was incorrect (void*)(pos+1) */
    return 0;
}
static int c_point(map_t*args)
{
    ...
    dict_put(&points, name, (char *)NULL + pos + 1); /* mieg 2010_10_18 was incorrect (void*)(pos+1) */
    return 0;
}
...
static int c_bubble (map_t*args)  /* mieg 2007_03_02, this macro creates a mouse-over bubble */
{
 //"name text"},
    ...
    int width = 2000 ;
    int height = 8000 ;
    RGBA color = parseColor("DARKBLUE");
    int maxlength = 0 ;
    char *variable =  "/:bubble_text" ;

    int align = -1;
    int flags = ET_WORDWRAP | ET_MULTILINE | ET_READONLY | ET_HTML | ET_BORDER | ET_NOSELECT | ET_AUTOSIZE | ET_ALIGN_LEFT;
...
{{"flash", c_flash, "bbox=autocrop background=black version=6 fps=50 name= filename= @compress=default @change-sets-all=no @export=1 @mainclass="},
 ...
 {"bubble", c_bubble, "name text"}, /* mieg 2007_03_02 aceview mouse over bubble */
...
int main (int argc,char ** argv)
{
    ...
    while(!noMoreTokens()) { /* mieg: 2007_01_17 added in parser.lex a command .exit, because EOF is hard to generate on a pipe */
	readToken();
	if(type != COMMAND)
	    syntaxerror("command expected");
