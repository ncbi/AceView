 ...
 *
 * SCCS: %W% %G%
 * Description: This is the file originally known as 'remote.c'
     which is the official sample implementation for the remote
     control of UNIX Netscape Navigator under X11.
     A few changes have been applied, e.g. the renaming of
     compiler flags by prepending NETSCAPE_REMOTE_ in order
     to keep the namespace of the graphlib cleaner.
 * Exported functions:
 * HISTORY:
 * Last edited: Oct 14 15:30 1998 (fw)
 * * Oct 14 15:11 1998 (fw): mozilla_remote_commands returns status 1
        if no exisiting netscape window could be found.
 * * Oct 14 15:03 1998 (fw): condotional for all exit() statements
        to be return or return <false> instead,
        if NETSCAPE_REMOTE_STANDALONE is not set.
	The exit()'s only make sense in a standalone program.
 * * Oct 14 14:01 1998 (fw): enclosed a few warnings (to stderr)
        in compiler flag-ifdefs NETSCAPE_REMOTE_WARNING
 * * Oct 14 13:33 1998 (fw): renamed the compiler flags ifdef
	STANDALONE to NETSCAPE_REMOTE_STANDALONE
	DEBUG_PROP to NETSCAPE_REMOTE_DEBUGPROP
        for cleaner namespace
 * * Oct 14 13:32 1998 (fw): enclosed the window locking mechanism in
        #ifdefs NETSCAPE_REMOTE_LOCK
 * * Oct 14 13:30 1998 (fw): dowloaded this file & include from netscape site
        http://home.netscape.com/newsref/std/remote.c
        http://home.netscape.com/newsref/std/vroot.h
 * Created: Wed Oct 14 13:29:18 1998 (fw)
 *-------------------------------------------------------------------
 */
/*****************************************************************************/
/* -*- Mode:C; tab-width: 8 -*-
 ...
 * implied warranty.
 *
 * To compile a standalone test-program in the w2/ directory
 *
 *    cc -o netscape_remote xnetscape_remote.c -I../wh -DNETSCAPE_REMOTE_STANDALONE -lXmu -lX11
 *
 *  Linux:
 *    gcc -o netscape_remote xnetscape_remote.c -I../wh -DNETSCAPE_REMOTE_STANDALONE -L/usr/X11R6/lib -lXmu -lX11
 *
 * To use:
 *
 *    netscape_remote -help
 *
...
#ifdef NETSCAPE_REMOTE_STANDALONE
 static const char *progname = 0;
 static const char *expected_mozilla_version = "1.1";
#define NETSCAPE_REMOTE_NAME ""
#else  /* !NETSCAPE_REMOTE_STANDALONE */
 extern const char *progname;
 extern const char *expected_mozilla_version;

/* The NETSCAPE_REMOTE_NAME string will be inserted in all
 *   warnings or errors written to stderr. This is because
 *  progname will probably the name of the software
 *  this file is compiled in. We also want to report
 *  the name of this module to avoid confusion */
#define NETSCAPE_REMOTE_NAME "netscape "
#endif /* !NETSCAPE_REMOTE_STANDALONE */
...
mozilla_remote_find_window (Display *dpy)
{
  ...
  if (! XQueryTree (dpy, root, &root2, &parent, &kids, &nkids))
    {
      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "XQueryTree failed on display %s\n", progname,
	       DisplayString (dpy));
#ifdef NETSCAPE_REMOTE_STANDALONE
      exit (2);
#else
  ...
  if (! (kids && nkids))
    {
      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "root window has no children on display %s\n",
  ...
  if (result && tenative)
    {
#ifdef NETSCAPE_REMOTE_WARNING
      fprintf (stderr,
	       "%s: " NETSCAPE_REMOTE_NAME "warning: both version %s (0x%x) and version\n"
...
#ifdef NETSCAPE_REMOTE_WARNING
      fprintf (stderr,
	       "%s: " NETSCAPE_REMOTE_NAME "warning: expected version %s but found version\n"
	       "\t%s (0x%x) instead.\n",
    ...
    {
      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "not running on display %s\n", progname,
	       DisplayString (dpy));
#ifdef NETSCAPE_REMOTE_STANDALONE
      exit (1);
#else
...
mozilla_remote_check_window (Display *dpy, Window window)
{
  ...
  if (status != Success || !version)
    {
      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "window 0x%x is not a Netscape window.\n",
    ...
    {
      fprintf (stderr,
	       "%s: " NETSCAPE_REMOTE_NAME "warning: window 0x%x is Netscape version %s;\n"
	       "\texpected version %s.\n",
	       progname, (unsigned int) window,
...
mozilla_remote_obtain_lock (Display *dpy, Window window)
{
  ...
  do
...
#ifdef NETSCAPE_REMOTE_DEBUG_PROPS
	  fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "(writing " MOZILLA_LOCK_PROP
		   " \"%s\" to 0x%x)\n",
      ...
      if (! locked)
	{
	  ...
	  fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "window 0x%x is locked by %s; waiting...\n",
	  ...
	  while (1)
		...
		{
		  fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "window 0x%x unexpectedly destroyed.\n",
			   progname, (unsigned int) window);
#ifdef NETSCAPE_REMOTE_STANDALONE
		  exit (6);
#else
		       ...
		       event.xproperty.atom == XA_MOZILLA_LOCK)
		{
		  /* Ok!  Someone deleted their lock, so now we can try
		     again. */
#ifdef NETSCAPE_REMOTE_DEBUG_PROPS
		  fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "(0x%x unlocked, trying again...)\n",
			   progname, (unsigned int) window);
#endif
		  break;
		}
  ...
  if (waited)
    fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "obtained lock.\n", progname);
}
...
mozilla_remote_free_lock (Display *dpy, Window window)
{
...
#ifdef NETSCAPE_REMOTE_DEBUG_PROPS
	  fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "(deleting " MOZILLA_LOCK_PROP
		   " \"%s\" from 0x%x)\n",
		   progname, lock_data, (unsigned int) window);
#endif
  ...
  if (result != Success)
    {
      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "unable to read and delete " MOZILLA_LOCK_PROP
	       " property\n",
	       progname);
      return;
    }
    ...
    {
      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "invalid data on " MOZILLA_LOCK_PROP
	       " of window 0x%x.\n",
	       progname, (unsigned int) window);
      return;
    }
    ...
    {
      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "" MOZILLA_LOCK_PROP
...
#endif /* ifdef NETSCAPE_REMOTE_LOCK */
			...
			Bool raise_p)
{
...
#ifdef NETSCAPE_REMOTE_DEBUG_PROPS
  fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "(writing " MOZILLA_COMMAND_PROP " \"%s\" to 0x%x)\n",
  ...
  while (!done)
	...
	{
	  /* Print to warn user...*/
	  fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "window 0x%x was destroyed.\n",
		   progname, (unsigned int) window);
	  result = 6;
	  goto DONE;
	}
	       ...
	       event.xproperty.atom == XA_MOZILLA_RESPONSE)
	{
	  ...
	  if (result == Success && data && *data)
	    {
	      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "(server sent " MOZILLA_RESPONSE_PROP
		       " \"%s\" to 0x%x.)\n",
	  ...
	  if (result != Success)
	    {
	      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "failed reading " MOZILLA_RESPONSE_PROP
		       " from window 0x%0x.\n",
		       progname, (unsigned int) window);
	      result = 6;
	      done = True;
	    ...
	    {
	      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "invalid data on " MOZILLA_RESPONSE_PROP
	    ...
	    {
	      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "%s\n", progname, data + 4);
	      /* keep going */
	      done = False;
	    }
#if 1
	    ...
	    {
	      fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "internal error: "
	    ...
	    {
	      fprintf (stderr,
		       "%s: " NETSCAPE_REMOTE_NAME "unrecognised " MOZILLA_RESPONSE_PROP
		       " from window 0x%x: %s\n",
		       progname, (unsigned int) window, data);
	      result = 6;
	      done = True;
	...
	{
	  fprintf (stderr, "%s: " NETSCAPE_REMOTE_NAME "(server 0x%x has accepted "
		   MOZILLA_COMMAND_PROP ".)\n",
		   progname, (unsigned int) window);
	}
#endif /* NETSCAPE_REMOTE_DEBUG_PROPS */
    }
