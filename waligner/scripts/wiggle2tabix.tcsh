...
ls -ls $iii
   if (! -e $iii) continue 
   echo "gunzip -c $iii | bin/wiggle -I BF -O BV |  gawk "'/^#/{next;}/^track/{next;}/^variableStep/{split($2,aa,"chrom=");zz=0;if(aa[2]==chrom)zz=1;next;}{if(zz==1)printf("%s\t%d\t%d\n",chrom,$1,$2);}'" chrom=$chrom1 | bgzip -c > tmp/TABIX/$group/$chrom.$unique.$ss.tabix.gz"
    gunzip -c $iii | bin/wiggle -I BF -O BV |  gawk '/^#/{next;}/^track/{next;}/^variableStep/{split($2,aa,"chrom=");zz=0;if(aa[2]==chrom)zz=1;next;}{if(zz==1)printf("%s\t%d\t%d\n",chrom,$1,$2);}' chrom=$chrom1 | bgzip -c > tmp/TABIX/$group/$chrom.$unique.$ss.tabix.gz
