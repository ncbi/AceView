...
if (-e tmp/TSNP_DB/$zone/tsnp2.done) goto w31
...
EOF
  ...
  foreach run (`cat MetaDB/$MAGIC/RunsList`)
    ...
    if (-e tmp/TSNP/$run/$zone/tsnp2.2.deUno.tsf) then
      cat tmp/TSNP/$run/$zone/tsnp2.2.deUno.tsf | gawk -F '\t' '/^#/{next;}{v=$1;run=$2;a1=$4;a2=$5;m=$7;c=$9;tag=$12;if(m>c)c=m;r=c-m;if(c>minC)f=100*m/c;else f=-10;if(f>=minF){if(v!=oldV){split(v,aa,":");seq=aa[1];printf("\nVariant %s\n%s\nParent_sequence %s\n%s %s %d %d\n%s\n",v,foundIn,seq,mapIn,seq,a1,a2,tag);oldV=v;printf("MCounts %s %d %d %d Frequency %.2f\n",run,m,r,c,f);}}}' minF=$minSnpFrequency2 minC=$minSnpCover foundIn=$foundIn mapIn=$mapIn >> $toto.ace
    endif
    if (-e tmp/TSNP/$run/$zone/tsnp2.MB.deUno.tsf) then
      cat tmp/TSNP/$run/$zone/tsnp2.MB.deUno.tsf | gawk -F '\t' '/^#/{next;}{v=$1;run=$2;a1=$4;a2=$5;m=$7;c=$9;if(m>c)c=m;tag=$12;r=c-m;if(c>minC)f=100*m/c;else f=-10;if(f>=minF){if(v!=oldV){split(v,aa,":");seq=aa[1];printf("\nVariant %s\n%s\nParent_sequence %s\%s %s %d %d\n%s\n",v,foundIn,seq,mapIn,seq,a1,a2,tag);oldV=v;printf("MBCounts %s %d %d %d Frequency %.2f\n",run,m,r,c,f);}}}' minF=$minSnpFrequency2  minC=$minSnpCover foundIn=$foundIn  mapIn=$mapIn >> $toto.ace
    endif    
  end
...
touch tmp/TSNP_DB/$zone/tsnp2.done
touch tmp/TSNP_DB/$zone/tsnp2.2.done
touch tmp/TSNP_DB/$zone/tsnp2.MB.done

w31:
# bin/tsnp -db tmp/TSNP_DB/$zone -p $MAGIC --makeWords --zone $zone -gzo -o tmp/TSNP_DB/$zone/tsnp2.$MAGIC --filter "MBcounts && substitution"
bin/tsnp -db tmp/TSNP_DB/$zone -p $MAGIC --makeWords --zone $zone -gzo -o tmp/TSNP_DB/$zone/tsnp2.$MAGIC 
