...
set tutu=tmp/GENERUNS/$run/$run.coverage_of_8kb_transcripts
if ($phase == m2bH && ! -e $tutu.txt.gz) then
    set ok=0
    echo "... constructing tmp/GENERUNS/$run/$run.3pHisto.txt"
    echo -n "## $run\tfile $tutu.txt\t" > $tutu.txt
    date	>> $tutu.txt
    echo ' ' > tmp/GENERUNS/$run/toto.$$
    foreach target (av $Etargets)
      if (-e tmp/METADATA/$target.selected8kbTranscriptList.txt) then
        cat  tmp/METADATA/$target.selected8kbTranscriptList.txt > tmp/GENERUNS/$run/toto1.$$
        foreach ff (`cat  tmp/GENERUNS/$run/$run.$target.$GM.3pHisto.list`)
          set ok=1
    ...
    if ($ok > 0) then
      cat  tmp/GENERUNS/$run/toto.$$ | gawk -F '\t' -f scripts/m2bH.3pHisto.awk nMax=$nMax > tmp/GENERUNS/$run/toto2.$$
