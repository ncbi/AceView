...
foreach target ($DNAtargets)
  if ($target == gdecoy) continue
  if (! -e  TARGET/Targets/$species.$target.TM.txt.gz) then
    echo "---- measuring the length of each sequence in DNA target : $target"
    (bin/dna2dna -i  TARGET/Targets/$species.$target.fasta.gz  -getTm -o  TARGET/Targets/$species.$target -gzo) >& tmp/METADATA/$target.TM.err
    gunzip -c TARGET/Targets/$species.$target.TM.txt.gz | wc
  endif
end

if (-e TARGET/Targets/$species.genome.TM.txt.gz) then
  gunzip -c  TARGET/Targets/$species.genome.TM.txt.gz | gawk -F '\t' '/^#/{next;}{printf("%s\t1\t%s\t1\t%d\t%s\n",$1,$1,$2,$1);}' >  tmp/METADATA/genome.ns.sponge.1
  gunzip -c  TARGET/Targets/$species.genome.TM.txt.gz | gawk -F '\t' '/^#/{next;}{printf("%sR\t1\t%s\t%d\t1\t%sR\n",$1,$1,$2,$1);}' >>  tmp/METADATA/genome.ns.sponge.1
  if (-e tmp/METADATA/genome.ns.sponge) \rm tmp/METADATA/genome.ns.sponge
  foreach chrom ($chromSetAll)
    cat tmp/METADATA/genome.ns.sponge.1  | gawk -F '\t' '{if($1 == chrom || $1 == chrom "R")print}' chrom=$chrom >> tmp/METADATA/genome.ns.sponge
  end
...
foreach target ($RNAtargets)
  ...
  set new_fasta=1
  if (! -e TARGET/Targets/$species.$target.TM.txt.gz) then
    (bin/dna2dna -i  TARGET/Targets/$species.$target.fasta.gz  -getTm -gzo -o TARGET/Targets/$species.$target) >>& tmp/METADATA/$target.TM.err
  endif
  gunzip -c TARGET/Targets/$species.$target.TM.txt.gz | gawk -F '\t' 'BEGIN{printf("#Transcript\tLength\tGC_percent\tGene\tGeneId\n");}/^#/{next;}{gsub(/>/,"",$1);split($1,aa,"|");t=aa[1];t=aa[1];g="";gid="";if(aa[2]=="Gene")g=aa[3];if(aa[4]=="GeneId")gid=aa[5];ln=$2;gc=$5;printf("%s\t%s\t%d\t%s\t%s\n",t,ln,gc,g,gid);}' | sort > tmp/METADATA/$target.mrna_ln_gc_gene_geneid.txt
...
set gtf_active=0
foreach target (RefSeq av $RNAtargets)
  source scripts/target2target_class.txt

  if (-e TARGET/GTF/$species.$target.gtf.gz || -e TARGET/GTF/$species.$target.gff.gz) then
    ...
    cat tmp/METADATA/gtf.$target.gene2intMap.txt | gawk -F '\t' '{printf("%s\t1\t%s\t%d\t%d\t%s\n",$1,$2,$3,$4,$1);}' > tmp/METADATA/$target.ns.gene.sponge
...
echo "$RNAtargets" >  tmp/METADATA/RNAtargets.new
...
# consruct  tmp/METADATA/mrnaRemap.gz 
  ...
  foreach target ($RNAtargets)
    if (-e tmp/METADATA/gtf.$target.gene2intMap.txt) then 
	 ...
	 zcat tmp/METADATA/gtf.$target.transcripts.ace.gz | gawk '/^$/{out=1;intron=0;next;}/^Intron/{if(out==1){out=0;intron=$2;}next;}/^Gene/{if(intron!=0){printf("Intron %s\nGene %s\n\n",intron, substr($0,5));}}' | gzip >   tmp/METADATA/toto.gz 
         ...
         zcat tmp/METADATA/gtf.$target.transcripts.ace.gz | gawk '/^Sequence/{s=$2;next;}/^Title/{printf("Sequence %s\nTitle %s\n\n",s, substr($0,7));}/^Model_of/{printf("Sequence %s\nModel_of %s\n\n",s, substr($0,7));}' >>  tmp/METADATA/$target.MRNA.info.ace
       endif
       gunzip -c tmp/METADATA/gtf.$target.gene_title.ace.gz >> tmp/METADATA/$target.GENE.info.ace
...
foreach target (av $Etargets)
  if ($ok == 0 && -e tmp/METADATA/$target.mrna_ln_gc_gene_geneid.txt && ! -e tmp/METADATA/$target.selected8kbTranscriptList.txt) then
...
######################################################
######################################################
# sponge files, backwards compatibility if we do not have the gtf file

echo "---- sponge files"
foreach target ($RNAtargets)
  foreach fr (f r ns)
    if (-e TARGET/WIGGLEREMAP/$target.$fr.sponge && ! -e tmp/METADATA/$target.$fr.sponge) then
      cp TARGET/WIGGLEREMAP/$target.$fr.sponge  tmp/METADATA
    endif
    ...
    if (-e TARGET/WIGGLEREMAP/$target'_cds'.$fr.sponge && ! -e tmp/METADATA/$target'_cds'.$fr.sponge) then
      cp TARGET/WIGGLEREMAP/$target'_cds'.$fr.sponge  tmp/METADATA
    endif
end

foreach target ($RNAtargets)
    if (-e TARGET/WIGGLEREMAP/mrnaRemap.$target.txt && ! -e TARGET/WIGGLEREMAP/$target.ns.sponge) then
      cat TARGET/WIGGLEREMAP/mrnaRemap.$target.txt | gawk -F '\t' '{m=$2;g=m;if($8)g=$8;if(m!=oldm)n=0;n++;oldm=m;a1=$6;a2=$7; if(a1<a2)printf("%s\t%d\t%s\t%d\t%d\t%s\n",m,n,$5,a1,a2,g);}' | sort -k 1,1 -k 2,2n  >  TARGET/WIGGLEREMAP/$target.f.sponge
      cat TARGET/WIGGLEREMAP/mrnaRemap.$target.txt | gawk -F '\t' '{m=$2;g=m;if($8)g=$8;if(m!=oldm)n=0;n++;oldm=m;a1=$6;a2=$7; if(a1>a2)printf("%s\t%d\t%s\t%d\t%d\t%s\n",m,n,$5,a1,a2,g);}' | sort -k 1,1 -k 2,2n  >  TARGET/WIGGLEREMAP/$target.r.sponge
      cat TARGET/WIGGLEREMAP/mrnaRemap.$target.txt | gawk -F '\t' '{m=$2;g=m;if($8)g=$8;if(m!=oldm)n=0;n++;oldm=m;a1=$6;a2=$7; printf("%s\t%d\t%s\t%d\t%d\t%s\n",m,n,$5,a1,a2,g);}' | sort -k 1,1 -k 2,2n  >  TARGET/WIGGLEREMAP/$target.ns.sponge
    endif
end
...
echo "---- known introns"
foreach target (magic $RNAtargets)
  ...
  foreach fr (f r)
    if (-e tmp/METADATA/$target.$fr.sponge && ! -e tmp/METADATA/$target.$fr.sponge_introns) then
...
foreach target ($RNAtargets) 
 ...
 if (-e  TARGET/GENES/$target.gene2title.ace) then
  ...
  if (-e TARGET/Targets/$species.$target.TM.txt.gz) then
    gunzip -c  TARGET/Targets/$species.$target.TM.txt.gz | gawk '{n=split($1,aa,"|");if(n>=5 && aa[2]=="Gene" && aa[4]=="GeneId")printf("Gene \"%s\"\nGeneId \"%s\"\n\n",aa[3],aa[5]);}' >>  tmp/METADATA/$target.GENE.info.ace
  endif

  if ($target == RefSeq && -e TARGET/Targets/$species.$target.TM.txt.gz) then
    gunzip -c  TARGET/Targets/$species.$target.TM.txt.gz | gawk '{n=split($1,aa,"|");if(n>=3 && aa[2]=="Gene" && substr($1,2,2) == "M_")printf("Gene \"%s\"\nPastille_coding\n\n",aa[3]);}' >>  tmp/METADATA/$target.GENE.info.ace
  endif
  ...
  \rm  tmp/METADATA/toto
     ...
     cat  tmp/METADATA/$target.metadata.txt | gawk -F '\t' '/^#/{next;}{n=split($6,aa,";");for(i=1;i<=n;i++){if(length($1)>0 && $4)printf("mRNA \"%s\"\nGene_type \"%s\"\n\n",aa[i],$4);if($5)printf("mRNA \"%s\"\nTitle \"%s\"\n\n",aa[i],$5);}}'  >>  tmp/METADATA/$target.MRNA.info.ace
