...
{ 
  ...
  for (j1 = 0 ; j1 < 4 ; j1++)
    { 
      ...
      sp = tt[j1] = (TRACE *) xmalloc(traceMax * sizeof (TRACE)) ;
...
{ 
  ...
  mixProb = (unsigned char *) xmalloc (read->NBases) ;
...
static void ctfUnmixProb (Read *read, int n, unsigned char *mixProb) 
{
  ...
  ca = (unsigned char *)(read->prob_A = (char *) xmalloc (n)) ;
  cc = (unsigned char *)(read->prob_C = (char *) xmalloc (n)) ;
  cg = (unsigned char *)(read->prob_G = (char *) xmalloc (n)) ;
  ct = (unsigned char *)(read->prob_T = (char *) xmalloc (n)) ;
...
{ 
  ...
  while (TRUE)
	...
	case 1:  /* read the bases  and positions, problem if negative dx */
	  ...
	  read->base = (char *)xmalloc (baseMax) ; /* staden's alloc */
	  b = (unsigned char *)read->base;
	  bp = read->basePos = (unsigned short *) xmalloc (baseMax * sizeof (unsigned short)) ;
	...
	case 11:  /* read the basecall, fullproof method */
	  ...
	  n = baseMax ; nn -= n ;
	  b = (unsigned char *)read->base = (char *)xmalloc (baseMax) ; 
	  memcpy (b, cp, n) ;
	  cp += n ;
	  break ;
	...
	case 4:  /*  Probability information */  
	  n = ctfGetInt (cp) ; cp += 4 ; nn -= 4 ;
	  read->prob_A = (char *) xmalloc (n) ;
	  read->prob_C = (char *) xmalloc (n) ;
	  read->prob_G = (char *) xmalloc (n) ;
	  read->prob_T = (char *) xmalloc (n) ;
	...
	case 5:  /*  Mixed Probability information */  
	  n = ctfGetInt (cp) ; cp += 4 ; nn -= 4 ;
	  mixProb = (unsigned char*) xmalloc (n) ; nMixProb = n ;
...
Read *ctfFRead (FILE *ff)
{
  ...
  if ((a = ctfReadTrace (ff)) &&
      ...
      if (ctfUnPackTraces (read, a))
	{
	  ...
	  if (!read->prob_A) 
	    {
	      read->prob_A = (char *) xmalloc (NBases) ;
	      if (!read->prob_C) read->prob_C = (char *) xmalloc (NBases) ;
	      if (!read->prob_G) read->prob_G = (char *) xmalloc (NBases) ;
	      if (!read->prob_T) read->prob_T = (char *) xmalloc (NBases) ;
