...
static Array ctfCompress3 (Array a)
{
  ...
  b = arrayCreate (3 *ia, char) ; /* worst case */
  array (b, 3*ia - 1 , unsigned char) = 0 ;  /* make room */
  cp = cp0 = arrp (b , 0, unsigned char) ;
  sp = arrp (a, 0, short) ;
  ...
  while (ia-- > 0)
      ...
      for (w = maxCode ; w >= 1 ; w--)  /* w-- favors long code words */
	{
      ...
      if (w >= 1 && lng[w] > 0 && lng[w] <= ia + 1) /* code word found */
	{
	  n2++ ; n10 += lng[w] ;  *cp++ = 0x80 | (w & 0x7f) ;
	  sp += lng[w] - 1 ; ia -= lng[w] - 1 ;
	  if (lng[w] <= 0)
	    {
  ...
  arrayMax(b) = cp - cp0 ;
  if (debug) 
    fprintf (stderr, " // compress3:\n//  %d zeros in %d bytes, %d values coded in %d byte, %d bytes, %d short. \n// Total %d char for %d shrt\n",
	  n0, n1, n10, n2, n3, n4, arrayMax(b), arrayMax(a)) ;
  if (arrayMax(a) != n0 + n10 + n3 + n4)
    { 
      fprintf (stderr, " // compress3:\n//  %d zeros in %d bytes, %d values coded in %d byte, %d bytes, %d short. \n// Total %d char for %d short != (%d + %d + %d + %d = %d) \n",
	       n0, n1, n10, n2, n3, n4, arrayMax(b), arrayMax(a),
	       n0, n10, n3, n4, n0 + n10 + n3 + n4) ;fprintf (stderr, "FATAL ERROR in ctfCompress3, coding error in compress 3") ;
      exit (1) ;
    }
  return b ;
}
...
static Array ctfCompress4 (Array a)
{
  BOOL debug = FALSE ;
  int i = arrayMax (a) ;
  Array b = arrayCreate (3 * i, unsigned char) ; /* worst case */
  unsigned char *cp, *cp0, cc, tt ;
  short *sp, z ;
  int z1, nb, nt, nbyte = 0, nshort = 0 ;
  int nbit[22] = { 8, 8, 8, 8,8,8,8,5,4,3,1,3,4,5,8,8,8,8,8, 8, 8, 8 } ;
  unsigned 
    code[22] = { 0xc1, /* -10 */
		 0xc2, /* -9 */
		 0xc3, /* -8 */
  ...
  while (i--)
    {
      z = *sp++ ;
      if (z > -11 && z < 11) /* Huffman code */
	z1 = z + 10 ;
      else
	z1 = 21 ; /* announce a byte */
  ...
  arrayMax(b) = cp - cp0 ;
  if (debug) 
    fprintf (stderr, " // compress4:\n//  %d values in %d bytes, %d bytes %d shorts\n",
	     arrayMax(a), arrayMax(b), nbyte, nshort) ;
...
{
  int j, j1, u0, u1, u2, u3, u4 ;
  short *zp, z = 0 ;
  TRACE *tt[4], *sp ;
  int pm0, traceMax = read->NPoints ;
  Array a = arrayCreate (4 * traceMax, short) ;
  Array h = arrayCreate (255,int) ;
  BOOL debug = FALSE ;
  ...
  for (j1 = 0 ; j1 < 4 ; j1++)
	...
	{ 
	  ...
	  switch (predictionMode)
	    ...
	    case 23:
	      {
		int pm, z1, z2, z3, z4, zmin ;
		z1 = u1 - u0 ;
		z2 = 2*u1 - u2  - u0 ;
		z3 = 3*u1 - 3*u2 + u3  - u0 ;
		z4 = 4*u1 - 6*u2 + 4*u3 - u4  - u0 ;
		zmin = z1 < 0 ? -z1 : z1 ; pm = 1 ;
		if (z2 >=0 && z2 < zmin) { zmin = z2 ; pm = 2 ; }
		if (z3 >=0 && z3 < zmin) { zmin = z3 ; pm = 3 ; }
		...
		switch (pm0)
		  {
		  case 1: z = z1 + u0 ; break ;
		  case 2: z = z2 + u0 ; break ;
		  case 3: z = z3 + u0 ; break ;
		  case 4: z = z4 + u0 ; break ;
		  case 0:
		  default: z = 0 ; break ;
		  }
		pm0 = pm ;
	    ...
	    case 103:
	      {
		int  z1, v1 ; /* value at location plus 2 */
		v1 = *(sp + 1) ;
		z1 = 5 * u1  - 6 * u2 + 2 * u3 ;
		*(zp+1) = v1 - z1 ;
		z = (4 * u0 - 2 * u1 +  v1)/3 ;	  
  ...
  if (1)
    {
      int i, n , nn = 0 ;
      double zz, zlog2 = log ((double)2.0); 
      int ztot = 0, nbittot = 0 ; 
      int nbit[23] = { 16, 8, 8, 8, 8,8,8,8,5,4,3,1,3,4,5,8,8,8,8,8, 8, 8, 16 } ;
      for (i = 0 ; i <= 22 ; i++)
	nn += array(h, i, int) ;
      for (i = 0 ; i <= 22 ; i++)
	{
	  zz = n = array(h, i, int) ;
	  zz /= nn ; zz = - log(zz) ; zz /= zlog2 ;
	  zz = n * zz/8 ;
	  if (debug) fprintf(stderr, "%4d,%6d %d %3d %4d \n",i - 11, n, nn, (int) zz, (n * nbit[i] / 8) ) ;
	  nbittot += n * nbit[i] ;
	  ztot += zz ;
	} 
      if (debug) fprintf(stderr, "ztotal = %d nbyttotal = %d nchar = %d\n", 
	      ztot, nbittot/8, nn) ;
    }
  arrayDestroy (h) ;
  return a ;
...
static BOOL baseCallSimpleGet (LANE *lane)
{
  ...
  if (read->basePos) free (read->basePos) ;
