...
int read_scf_header(FILE *fp, Header *h)
{
    int i;
    /* int mySCFmagic  ;
       memcpy(&mySCFmagic,".scf",4) ;
    
      this is a bug, be_read_int_4 calls 
      #define be_int4(x) iswap_int4((x))
      and you never want to swap the signature
    */
      if (be_read_int_4(fp,&h->magic_number)==False)        
      return -1;
      /*
    if (fread(&(h->magic_number), 4, 1, fp) != 1) 
      return -1 ;
      */
      if (h->magic_number != SCF_MAGIC)   /* mySCFmagic) */
	{
	  char buf[5], bufm[5] ;
	  int m = SCF_MAGIC ;
	  memcpy (buf, &(h->magic_number), 4) ;
	  buf[4] = 0 ;  
	  memcpy (bufm, &m, 4) ;
	  bufm[4] = 0 ;
	  fprintf(stderr, "bad ctf magic %s != %s  in read_scf_header", buf, bufm) ;
	  return -1;
	}
...
int read_scf_samples32(FILE *fp, Samples2 *s, size_t num_samples) {
    ...
    if ( ! (samples_out = (uint2 *)xmalloc(num_samples * 
					    sizeof(uint2)))) {
	return -1;
    }
...
Scf *fread_scf(FILE *fp) {
	...
	if (-1 == fseek(fp, (off_t)(h.private_offset), 0) ||
	    -1 == fread(scf->private_data, 1, h.private_size, fp)) {
	    scf_deallocate(scf);
	    return NULL;
	}
    }
