 ...
 *   1 7s >15h killed
 *
 * i need to see  if it comes from aql or from aceC
 */
#ifdef JUNK
static int giwAddIntronClass (AC_DB db, KEY key)
{
   AC_HANDLE h = ac_new_handle () ;
   int nn = 0 ;
   AC_TABLE tbl = 0, tblPg ;
   KEY est, oldEst = 0 ;
   vTXT qry = vtxtHandleCreate (h) ;
   vTXT qryPg = vtxtHandleCreate (h) ;
   vTXT bfr = vtxtHandleCreate (h) ;

   const char *ccp ;
   char *tgNam, *cp ;
   int ir, i1, i2, tg1, tg2, u1, u2 ;
   int jr, x1, x2, pg1, pg2 ;
   int old1 = 0, old2 = 0, oldtg1 = 0, oldtg2 = 0 ;
   int nest = 0, nnm = 0, nmrna = 0, nja = 0, njb = 0, nPg = 0 ;

   vtxtPrintf (qry, " select tg, i1, i2, tgm, tg1, tg2, type, est, refseq, refmrna") ; /* tissue */
   vtxtPrintf (qry, " from g in class gene where g.name like %s", ac_protect(name(key), h)) ;
   vtxtPrintf (qry, " , tg in g->transcribed_gene where exists tg and exists_tag tg->Intron_boundaries") ;
   vtxtPrintf (qry, " , tgm in tg->intmap, tg1 in tgm[1], tg2 in tg1[1] ") ;
   vtxtPrintf (qry, " , type in tg->Intron_boundaries" ) ;
   vtxtPrintf (qry, " , nt in type[1]") ;
   vtxtPrintf (qry, " , i1 in nt[1], i2 in i1[1], est in i2[1]") ;
   vtxtPrintf (qry, " , refseq in exists_tag est->Ref_seq") ;
   vtxtPrintf (qry, " , refmrna in exists_tag est->Ref_mrna") ;
   /* vtxtPrintf (qry, " , tissue in est->tissue") ; */
   
   vtxtPrintf (qryPg, " select pg, x1, x2, pgm, pg1, pg2 ") ;
   vtxtPrintf (qryPg, " from g in class gene where g.name like %s", ac_protect(name(key), h)) ;
   vtxtPrintf (qryPg, " , pg in g->genefinder ") ;
   vtxtPrintf (qryPg, " , pgm in pg->intmap, pg1 in pgm[1], pg2 in pg1[1] ") ;
   vtxtPrintf (qryPg, " , x1 in pg->Source_Exons, x2 in x1[1]") ;
   
   tbl = ac_aql_table (db, vtxtPtr (qry), 0, 0, 0, h) ;
   tblPg = ac_aql_table (db, vtxtPtr (qryPg), 0, 0, 0, h) ;


   for (ir = 0 ; tbl && ir < tbl->rows ; ir++)
     {
       if (!ac_table_printable (tbl, ir, 0, 0))
	 continue ;
       i1 = ac_table_int (tbl, ir, 1, 0) ;
       i2 = ac_table_int (tbl, ir, 2, 0) ;
       tg1 = ac_table_int (tbl, ir, 4, 0) ;
       tg2 = ac_table_int (tbl, ir, 5, 0) ;
       if (i1 != old1 || i2 != old2 || tg1 != oldtg1 || tg2 != oldtg2)
	 {
	   nn++ ;
	   if (nPg) vtxtPrintf (bfr, "Pg %d\n", nPg) ; 
	   if (nnm) vtxtPrintf (bfr, "NM %d\n", nnm) ; 
	   if (nmrna) vtxtPrintf (bfr, "mRNA %d\n", nmrna) ; 
	   if (nest) vtxtPrintf (bfr, "EST %d\n", nest) ; 
	   if (nja) vtxtPrintf (bfr, "Brain %d\n", nja) ; 
	   if (njb) vtxtPrintf (bfr, "Stratagene %d\n", njb) ; 

	   tgNam = ac_protect(ac_table_printable (tbl, ir, 0, 0), h) ;
	   cp = tgNam + strlen (tgNam) - 1 ; *cp = 0 ;
	   vtxtPrintf (bfr, "\n Intron %s__%d_%d\n", tgNam, i1, i2) ;

	   ccp = ac_table_printable (tbl, ir, 0, 0) ;
	   vtxtPrintf (bfr, "From_gene %s %d %d\n",  ac_protect(ccp, h), i1, i2) ; 

	   if (tg1 < tg2) { u1 = tg1 + i1 - 1; u2 = tg1 + i2 - 1 ; }
	   else { u1 = tg1 - i1 + 1; u2 = tg1 - i2 + 1 ; }
	   ccp = ac_table_printable (tbl, ir, 3, 0) ;
	   vtxtPrintf (bfr, "IntMap %s %d %d\n", ac_protect(ccp,h), u1, u2) ; 
	   
	   ccp = ac_table_printable (tbl, ir, 6, 0) ;
	   if (ccp) vtxtPrintf (bfr, "Type %s\n", ccp) ;

	   old1 = i1 ; old2 = i2 ; oldtg1 = tg1 ; oldtg2 = tg2 ;
	   nnm = nmrna = nest = nja = njb = nPg = 0 ; oldEst = 0 ;

	   for (jr = 0 ; tblPg && jr < tblPg->rows - 1 ; jr++)
	     {
	       pg1 = ac_table_int (tblPg, jr, 4, 0) ;
	       pg2 = ac_table_int (tblPg, jr, 5, 0) ;
	       x1 = ac_table_int (tblPg, jr, 2, 0) ;
	       x2 = ac_table_int (tblPg, jr+1, 1, 0) ;
	       if (
		   (pg1 < pg2 && pg1 + x1 - 1 + 1 == u1 &&  pg1 + x2 - 1 - 1 == u2) ||
		   (pg1 > pg2 && pg1 - x1 + 1 - 1 == u1 &&  pg1 - x2 + 1 + 1 == u2)
		   )
		 nPg++ ;
	     }
	 }
       est = ac_table_key (tbl, ir, 7, 0) ;
       if (est != oldEst)
	 {
	   oldEst = est ;
	   ccp = ac_table_printable (tbl, ir, 8, 0) ;
	   if (*ccp == 'T') nnm++ ;
	   else
	     {
	       ccp = ac_table_printable (tbl, ir, 9, 0) ;
	       if (*ccp == 'T') nmrna++ ;
	       else 
		 {
		   nest++ ;
		   ...
		   if (ccp[0] == 'J' && ccp[2] == 'A' && ccp[4] == '_') nja++ ;
		   if (ccp[0] == 'J' && ccp[2] == 'B' && ccp[4] == '_') njb++ ;
		 }
	     }
	 }
       if (tbl->cols > 10)
