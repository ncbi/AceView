...
/* $Id: aceclientlib.c,v 1.2 2017/01/19 21:53:26 mieg Exp $ */
...
/* check if request contains a local command */
  ...
  */
  ...
  /* APL - We'll have no more of this 'for(0..length) answer[i]=loop[i];' nonsense! */
  memcpy(answer, loop, length);
  answer[length] = 0 ; /* zero terminate */
  xdr_free((xdrproc_t )xdr_ace_reponse, (char *)reponse);
  *answerPtr = answer;
  *answerLength = length;
  *encorep = encore ;
...
char * askServer (ace_handle *handle, char *request, int *lengthPtr) 
{ 
  int len2, encore=0, length ;
  int returnValue;
  char *answerPtr;
  int chunkSize = 0;
  unsigned char *binaryAnswer;
  char *requests, *arequest;

  /* botching multi-line commands till server is fixed */
  requests = (char *) malloc (strlen(request)+1);
  strcpy(requests, request);
  arequest = strtok(requests, "\n");
  returnValue = askServerBinary(handle, arequest, &binaryAnswer, &length, &encore, chunkSize) ;
  length--;
  if (returnValue > 0) return 0;

  answerPtr = (char *) malloc(length+1);
  memcpy(answerPtr, (char *) binaryAnswer, length);
  free (binaryAnswer);

  if (encore > 0) arequest = "encore";
  else arequest  =  strtok(NULL,"\n"); 

  while (arequest) {
    returnValue = askServerBinary(handle, arequest, &binaryAnswer, &len2, &encore, chunkSize) ;
    len2--;
    /*it'll timeout so lets just bail on error!  */
    if (returnValue > 0) break;
    answerPtr = realloc (answerPtr, length + len2+2);
    memcpy(answerPtr+length, (char *) binaryAnswer, len2);
    free(binaryAnswer);
    length += len2;
    if (encore > 0) arequest = "encore";
    else arequest = strtok(NULL,"\n");
