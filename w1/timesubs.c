...
/* $Id: timesubs.c,v 1.9 2017/03/23 20:36:08 mieg Exp $ */
...
mytime_t timeParse (char *ace_time) 
{
  ...
  char unit ;
  char *cp = ace_time;
  int x, v, n ;    /* number of chars read so far */
  ...
  switch ((int) unit)
    ...
    case 0:
      ts.tm_year = x ;
      cp-- ;  /* we went over the string end */
      unit = 'y' ;
      break ;
    case '-':
      ts.tm_year = x ;
  ...
  if (unit == 'y')
    {
      if (0 && ts.tm_year < 1900)	/* convert into 4 digit-year */
	{ if (ts.tm_year > 50) 
	    ts.tm_year += 1900 ;
	  else               
	    ts.tm_year += 2000 ;
...
char *timeShowJava (mytime_t t, char *buf, int bufsize)
{
  ...
  if (!buf || bufsize < 1) 
    messcrash ("timeShowJava received a null buf") ;
  if (!t)
    {
  ...
  strncpy (buf, ace_time, bufsize - 1) ;
  return buf ;
}

/**********************************************/

char *timeShow (mytime_t t, char *buf, int bufsize)
{
  char ace_time[25] ;
  struct tm ts;
  BOOL wantMonth, wantDay, wantHours, wantMins, wantSecs;
  ...
  if (!buf || bufsize < 1) 
    messcrash ("timeShow received a null buf") ;
  if (!t)
    {
      *buf = 0 ; return buf ; 
    }
...
char *timeBufShowNow (char *timeBuf)
{
  return timeShow (timeNow(), timeBuf, 25) ;
}

char *timeShowNow (void)
{
  static char timeBuf[25] ; /* NOT REENTRANT, Deprecated, prefer timeHandleShowNow (AC_HANDLE h) */
...
BOOL timeDiffSecs (mytime_t t1, mytime_t t2, int *diff) 
{
  ...
  wantMonth1 = wantDay1 = wantHours1 = wantMins1 = wantSecs1 = FALSE ;
  wantMonth2 = wantDay2 = wantHours2 = wantMins2 = wantSecs2 = FALSE ;
...
/* Suzy Lewis added a more general version to format data strings */
